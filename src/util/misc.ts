    ...
    switch (camel) {
        ...
        case "dromedary":
            return text
                ...
                .replace(/\s+/g, "");
        default:
            throw ErrorExitNihilo("I don't think that's a real camel.");
    }
}
...
function parseHTML(unparsed: string): HTMLElement {
    ...
    if (!(element instanceof HTMLElement)) throw ErrorExitNihilo("Unexpected error parsing HTML");
...
function ErrorExitNihilo(message: string): Error {
    return Error(`ExitNihilo System | ${message}`);
}

/** Returns the number in an ordinal format, like 1st, 2nd, 3rd, 4th, etc */
function ordinal(value: number) {
    const suffixes = LocalizeExitNihilo.translations.EXITNIHILO.OrdinalSuffixes;
    const pluralRules = new Intl.PluralRules(game.i18n.lang, { type: "ordinal" });
    const suffix = suffixes[pluralRules.select(value)];
    return game.i18n.format("EXITNIHILO.OrdinalNumber", { value, suffix });
}

/** Localizes a list of strings into a comma delimited list for the current language */
function localizeList(items: string[]) {
    const parts = LocalizeExitNihilo.translations.EXITNIHILO.ListPartsOr;
